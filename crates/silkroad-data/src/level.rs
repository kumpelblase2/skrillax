...
impl FromStr for RefLevel {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        Ok(Self {
            ...
            mob_exp: elements.get(5).ok_or(ParseError::MissingColumn(5))?.parse()?,
            job_exp_trader: elements.get(6).ok_or(ParseError::MissingColumn(6))?.parse()?,
            job_exp_thief: elements.get(7).ok_or(ParseError::MissingColumn(7))?.parse()?,
            job_exp_hunter: elements.get(8).ok_or(ParseError::MissingColumn(8))?.parse()?,
            pet_exp: elements.get(9).ok_or(ParseError::MissingColumn(9))?.parse()?,
            pet_stored_sp: elements.get(10).ok_or(ParseError::MissingColumn(10))?.parse()?,
        })
    }
}
